...
class MolecularDataset(Dataset):
    def __init__(self, sources=[], props=["logIC50", "BFL", "pipeline"], with_missings=False):
        ...
        for source_descr in sources:
            ...
            for i, prop in enumerate(props):
                if prop in source_descr:
                    if isinstance(source_descr[prop], str):
                        cur_props[:, i] = torch.from_numpy(cur_df[source_descr[prop]].values)
                    else:
                        cur_props[:, i] = torch.from_numpy(
                            cur_df[source_descr["smiles"]].map(source_descr[prop]).values
                        )
                else:
                    cur_missings[:, i] = 1
            ...
            self.len = max(self.len, int(len(cur_smiles) / source_descr["prob"]))
